...
# read data
hairdryer = pd.read_excel('../dataset/hairdryer_clean.xlsx')
microwave = pd.read_excel('../dataset/microwave_clean.xlsx')
pacifier = pd.read_excel('../dataset/pacifier_clean.xlsx')
...
text_fit(hairdryer.loc[:, 'review'][hairdryer['vine'] == 'Y'], y1[hairdryer['vine'] == 'Y'], tfidf_n,
         LogisticRegression(), coef_show=1)
text_fit(microwave.loc[:, 'review'][microwave['vine'] == 'Y'], y2[microwave['vine'] == 'Y'], tfidf_n,
         LogisticRegression(), coef_show=1)
text_fit(pacifier.loc[:, 'review'][pacifier['vine'] == 'Y'], y3[pacifier['vine'] == 'Y'], tfidf_n, LogisticRegression(),
         coef_show=1)
...
def caculatescore(df):
    ...
    df['score'] = (df['total_sentiscore'] + df['star']) * df['helping_rate'] * (
                df['vine'] + df['verified_purchase']) / 2
...
pacifier['total_sentiscore'][
    pacifier['product_title'] == 'philips avent bpa free contemporary freeflow pacifier'].groupby(
    pacifier['date']).mean().plot()
pacifier['star_rating'][pacifier['product_title'] == 'philips avent bpa free contemporary freeflow pacifier'].groupby(
    pacifier['date']).mean().plot()
...
stats.stats.pearsonr(pacifier['star_rating'].groupby(pacifier['date']).mean().values,
                     pacifier['total_sentiscore'].groupby(pacifier['date']).mean().values)
stats.stats.pearsonr(hairdryer['star_rating'].groupby(hairdryer['date']).mean().values,
                     hairdryer['total_sentiscore'].groupby(hairdryer['date']).mean().values)
stats.stats.pearsonr(microwave['star_rating'].groupby(microwave['date']).mean().values,
                     microwave['total_sentiscore'].groupby(microwave['date']).mean().values)
stats.stats.pearsonr(pacifier['star_rating'].groupby(pacifier['date']).mean().values[:-1],
                     pacifier['total_sentiscore'].groupby(pacifier['date']).mean().values[1:])
stats.stats.pearsonr(hairdryer['star_rating'].groupby(hairdryer['date']).mean().values[:-1],
                     hairdryer['total_sentiscore'].groupby(hairdryer['date']).mean().values[1:])
stats.stats.pearsonr(microwave['star_rating'].groupby(microwave['date']).mean().values[:-1],
                     microwave['total_sentiscore'].groupby(microwave['date']).mean().values[1:])